...
if (( ${#Files} )); then
    nice tar \
        ...
        --create \
        --file \
        "MYSQL_-_"$Database"_-_"$DayDateTime"_-_WithoutArchives.sql.tar.bz2" *.sql 2>/dev/null;
fi
...
if [[ $HourNumber == $HourCompleteBackupWebsite ]]; then
    ...
    if (( ${#Files} )); then
        nice tar \
            ...
            --create \
            --file \
            "MYSQL_-_"$Database"_-_"$DayDateTime"_-_Archives.sql.tar.bz2" *.sql 2>/dev/null;
    fi
fi
